...
class NBAM(nn.Module):
    def __init__(self, channels, kernel_size=7):
        super(NBAM, self).__init__()
        self.channel_attention = NAM_easy(channels)
        self.spatial_attention = SpatialAttention_easy(kernel_size)
...
class NAM_easy(nn.Module):
    def __init__(self, channels):
        super(NAM_easy, self).__init__()
...
class NAM_easy(nn.Module):
    ...
    def forward(self, x):
        ...
        # Step1: 动态计算当前batch的统计量
        if self.training:
            # 沿Batch,H,W维度计算均值方差 [1,C,1,1]
            mean = x.mean(dim=[0, 2, 3], keepdim=True)
            var = x.var(dim=[0, 2, 3], keepdim=True, unbiased=False)

            # 更新滑动平均统计量 (参考网页3/6的实现)
        ...
        # Step3: 使用gamma生成通道注意力权重 (网页11核心思想)
        mc = self.gamma * x_norm  # 缩放因子作用于归一化后的特征
        mc = torch.sigmoid(mc)  # 非线性激活

        return mc * residual  # 残差连接
        
'''

# 定义 CBAM 的空间注意力部分
class SpatialAttention_easy(nn.Module):
    def __init__(self, kernel_size=7):
        super(SpatialAttention_easy, self).__init__()
        assert kernel_size in (3, 7), 'kernel size must be 3 or 7'
        padding = 3 if kernel_size == 7 else 1
    ...
    def forward(self, x):
        ...
        x = torch.cat([max_out, avg_out, max_out], dim=1)
        x = self.conv(x)
        return x * self.sigmoid(x)
